...
abstract contract HelperContract is IDiamond, IDiamondLoupe, Test{
    using strings for *;
    string[] facetArray = ['sdData','sdInitialize','sdDepositFunds','sdSystem','sdMigration'];

    function loadFacets(address _diamond) public {
        bytes4[] memory rv;
        for(uint i = 0; i < facetArray.length; i++) {
            emit log(facetArray[i]);
            rv = generateSelectors(facetArray[i]);
            for(uint t = 0;t<rv.length;t++) {
                emit log_bytes(abi.encodePacked(rv[t]));
            }
        }
